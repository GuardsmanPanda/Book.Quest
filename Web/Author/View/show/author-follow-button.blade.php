<div hx-target="this" hx-swap="outerHTML">
    @if($status === 'FOLLOW')
        <button type="button" hx-patch="/author/{{$author_id }}/user-status"  hx-vals='{"status":"DEFAULT", "_token":"{{ csrf_token() }}"}'
    ...
    @else
        <button type="button" @if(\GuardsmanPanda\Larabear\Infrastructure\App\Service\BearGlobalStateService::getUserId() === null)  onclick="dialog('/login/login-selector-dialog')" @else hx-patch="/author/{{$author_id }}/user-status" hx-vals='{"status":"FOLLOW", "_token":"{{ csrf_token() }}"}' @endif
